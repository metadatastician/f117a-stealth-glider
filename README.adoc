...
*https://metadatastician.github.io/f117a-stealth-glider/[▶ Play it]* —
...
Sibling to https://github.com/metadatastician/f19-stealth-glider[`f19-stealth-glider`],
...
_disturbing_ it. This one adds the thing the real F-117A was actually built
...
21/22/23/25 turns. A _canned_ route dies off-phase; a _steered_ one never has
to. Reading the automaton is mandatory *and always sufficient*.
____

The gate got _stronger_ before the level passed it. The majority assertion was
...
generation from LOCK; the only freedom left is _when_, and that is the game.

What closed it, each step measured (full history in the ledger): the wall had
to _be_ a wall (0/30 → the chokepoint); shutter *tangency* beats coverage
...
audited sources. See link:./VERIFICATION.adoc[`VERIFICATION.adoc`],
link:./AUDIT.adoc[`AUDIT.adoc`] and link:./ARCHITECTURE.adoc[`ARCHITECTURE.adoc`].
...
glider _ecology_: 180 random soups per rule on a 24³ torus, 160 generations,
...
The v0 3D prototype is preserved verbatim in link:./spike/[`spike/`] with its seven
...
|link:./VERIFICATION.adoc[`VERIFICATION.adoc`] |The ledger. Every claim cites a script you can run. *Start here.*
|link:./AUDIT.adoc[`AUDIT.adoc`] |What is verified, what is argued, and what is *not* claimed.
|link:./ARCHITECTURE.adoc[`ARCHITECTURE.adoc`] |The layers, and why the world is 2D while the camera is 3D.
|link:./KERNEL-DIVERGENCE.adoc[`KERNEL-DIVERGENCE.adoc`] |Exactly how this reuses f19's kernel, and how that is enforced.
|link:./spike/README.md[`spike/README.md`] |The 3D prototype, and its seven defects with line cites.
|===

== Licence

link:./LICENSE[MPL-2.0] for code and the `.a2ml` metadata; `CC-BY-SA-4.0` for
documentation (relicensed from AGPL-3.0-or-later on 2026-09-02, owner ruling). Texts vendored under link:./LICENSES[`LICENSES/`].
