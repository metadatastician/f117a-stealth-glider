...
//
// MEASURED BUG, fixed here: pressing any direction key threw the view into a
// spin — 1135 degrees of yaw in a SINGLE frame, flying the witness route.
//
// The cause is derivative kick, the classic failure of a PID given a step
// setpoint. A glider's heading changes INSTANTLY at phase 0, so on a turn:
//   * wantYaw steps by ~90 degrees, and
//   * the chase point swings to the other side of the aircraft — `want` moves
//     by up to 2*BACK = 26 cells between one frame and the next.
// The derivative term is (error - prevError)/dt, so at 60 fps that step is
// multiplied by ~60. The controller commands an enormous rate, overshoots,
// the error flips sign, and it oscillates: the view flashes between angles.
//
// The controllers themselves are NOT the bug and are left faithful to
// spike/simulation.html. The fix is to stop handing them a discontinuity —
// an airframe's COMMANDED attitude does not teleport either — and to bound
// what they can ask for. TURN_SMOOTH eases the commanded direction; the MAX_*
// clamps mean that even an unforeseen step cannot produce a spin. dt is
// clamped too, because at very high frame rates a small dt inflates every
// derivative.
const TURN_SMOOTH = 3.5;        // 1/sec — commanded direction easing
const MAX_ACC = 140;            // cells/sec^2
const MAX_VEL = 45;             // cells/sec
const MAX_YAW_RATE = 2.6;       // rad/sec — about 150 deg/sec
const MAX_DT = 1 / 30;          // sec — bound the derivative at any frame rate
...
export function createCamera(L) {
  ...
  const cam = {
    ...
    // pursuit state
    vel: [0, 0, 0],
    // The COMMANDED direction, eased toward the glider's actual heading. The
    // glider turns in one tick; the camera's idea of "behind" does not.
    dir: [hx / n, hy / n],
...
export function stepCamera(cam, target, heading, dt) {
  ...
  // Ease the COMMANDED direction toward the actual heading rather than
  // snapping to it. This is what keeps a step out of the controllers: see the
  // derivative-kick note above the constants.
  const k = Math.min(1, TURN_SMOOTH * dt);
  cam.dir[0] += (heading[0] / n - cam.dir[0]) * k;
  cam.dir[1] += (heading[1] / n - cam.dir[1]) * k;
  const dn = Math.hypot(cam.dir[0], cam.dir[1]) || 1;
  const dir = [cam.dir[0] / dn, cam.dir[1] / dn];
  const want = [
  ...
  // in the camera as kicks; the clamps keep any residual step from becoming
  // a spin.
  const acc = [
    clamp(cam.pidX.compute(want[0], cam.pos[0], dt), -MAX_ACC, MAX_ACC),
    clamp(cam.pidY.compute(want[1], cam.pos[1], dt), -MAX_ACC, MAX_ACC),
    clamp(cam.pidZ.compute(want[2], cam.pos[2], dt), -MAX_ACC, MAX_ACC),
  ];
  for (let i = 0; i < 3; i++) {
    cam.vel[i] = clamp((cam.vel[i] + acc[i] * dt) * Math.max(0, 1 - 2.2 * dt), -MAX_VEL, MAX_VEL);
  ...
  const yawRate = clamp(cam.pidYaw.compute(0, angDiff(cam.yaw, wantYaw), dt), -MAX_YAW_RATE, MAX_YAW_RATE);
  cam.yaw += yawRate * dt;
